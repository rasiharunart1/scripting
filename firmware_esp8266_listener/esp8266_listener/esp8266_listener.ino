/***************************************************
 *  esp8266_listener.ino  — v2.0
 *
 *  ESP8266 STANDALONE SENSOR LISTENER & ALERT SYSTEM
 *  ──────────────────────────────────────────────────
 ...
 *       c. Aktifkan buzzer/LED alert jika ada yang OOR
 *
 ...
 *  D3 (GPIO0 ) → Button FLASH        (silence / restart)
 *
 ***************************************************/
...
// ════════════════════════════════════════════════════════════
//  HTTP CLIENT
//  Pakai BearSSL untuk HTTPS, plain WiFiClient untuk HTTP
// ════════════════════════════════════════════════════════════
BearSSL::WiFiClientSecure secureClient;

// ── Helper: buat HTTPClient + begin (auto deteksi http/https) ──
// Kembalikan false jika gagal
bool httpBegin(HTTPClient& http, const String& url) {
  http.setTimeout(HTTP_TIMEOUT_MS);
  if (url.startsWith("https://")) {
    secureClient.setInsecure();  // skip certificate verify (cocok ESP8266 RAM terbatas)
    secureClient.setBufferSizes(2048, 1024);
    return http.begin(secureClient, url);
  } else {
    // HTTP biasa: gunakan WiFiClient lokal (harus di heap agar tidak dangling)
...
//  BUZZER HELPERS
// ════════════════════════════════════════════════════════════
void buzzerOn()  { digitalWrite(BUZZER_PIN, LOW);  buzzerState = true;  }
void buzzerOff() { digitalWrite(BUZZER_PIN, HIGH); buzzerState = false; }
...
//  (Struktur sesuai SensorController::getSensorData)
// ════════════════════════════════════════════════════════════
...
void setup() {
  ...
  Serial.println(F("\n╔══════════════════════════════════════════╗"));
  Serial.println(F("║   ESP8266 Listener + Alert System v2.0  ║"));
  Serial.println(F("║   HTTP Polling → Laravel API             ║"));
  Serial.println(F("╠══════════════════════════════════════════╣"));
  Serial.printf( "║  API  : %-33s║\n", LARAVEL_BASE_URL);
...
void loop() {
  ...
  if (now - tBlink >= blinkMs) {
    tBlink   = now;
    ledState = !ledState;
    // Saat buzzer blink sudah handle LED jika alert, hindari konflik
    if (!alertActive || silenced) {
      digitalWrite(LED_PIN, ledState ? LOW : HIGH);
    }
  }
